# Türkiye Vergilendirme Rehberi

## Content

1. [Giriş](#giriş)
2. [Sözlük](#sözlük)
3. [Tanimlar](#tanimlar)
4. [Araçlar](#araçlar)
    1. [Hisse Senedi](#hisse-senedi)
    2. [Kripto](#kripto)
    3. [Yatırım Fonu](#yatırım-fonu)
5. [Vergilendirme](#vergilendirm)
6. [Kâr Hesaplama](#kâr-hesaplama)

## Giriş 
Buradaki tüm anlatılanlar bilgilendirme amaçlıdır. Bu bilgiler ışığında oluşan herhangi bir yasal sorun tamamen bu bilgileri kullanan kişinin sorumluluğundadır! Ayrica belge bireysel yatirimcilar icin hazirlanmistir, bu işi **ticari bir organizasyon** düzeyinde profesyonel meslek olarak yapiyorsaniz farkli bir vergilendirmeye tabii olabilirsiniz.

Buradaki bilgiler 193 sayili [Gelir Vergisi Kanunu ](https://www.mevzuat.gov.tr/mevzuat?MevzuatNo=193&MevzuatTur=1&MevzuatTertip=4), 213 sayili [Vergi Usul Kanunu](https://www.mevzuat.gov.tr/mevzuat?MevzuatNo=213&MevzuatTur=1&MevzuatTertip=4) ve 6362 sayili [Sermaye Piyasası Kanunu](https://mevzuat.gov.tr/mevzuat?MevzuatNo=6362&MevzuatTur=1&MevzuatTertip=5) isiginda hazirlanmistir. Bu belgede;

* Gelir Vergisi Kanununu `GVK`, 
* Gelir İdaresi Başkanlığı (İnternet Sitesi) `GİB`,
* Sermaye Piyasasi Kanununu `SPK` ,
* Vergi Usul Kanunu `VUK`,
* Türkiye Cumhuriyet Merkez Bankası `TCMB`

seklinde kisaltilmis olarak kullanilacaktir. Kanunlar 5 Mart 2026 tarihindeki hali ile kullanilmistir.

GİB uzerinden vergi vermek ile ilgili daha detayli bilgiye ulasmak icin [Link](https://intvrg.gib.gov.tr/hazirbeyan/assets/pdf/HBS_KULLANICI_KILAVUZU_2025.pdf) inceleyin.


## Sözlük

1. **İrat:** Gelir
2. **Pay:** Hisse Senedi
3. **Menkul:** Tasinabilen mal varliklari
4. **Gayri-:** ön ektir. Olmayan anlami katar.(Ex: Gayrimenkul = tasinamaz mal varliklari)
5. **Tevkifat:** Kesinti, stopaj
    


## Tanimlar

1. **Menkul kıymetler:** 
    > Para, çek, poliçe ve bono hariç olmak üzere: 
    > 1. Paylar, pay benzeri diğer kıymetler ile söz konusu paylara ilişkin depo sertifikalarını,
    > 2. Borçlanma araçları veya menkul kıymetleştirilmiş varlık ve gelirlere dayalı
borçlanma araçları ile söz konusu kıymetlere ilişkin depo sertifikalarını,

    > ifade etmektedir. `SPK M. 3, o`

    ***

2. **Türev araçlar:** 
    >Aşağıda sayılan veya Kurulca bu kapsamda olduğu belirlenen diğer
türev araçları:
    > 1. Menkul kıymetleri satın alma veya satma veya birbirleri ile değiştirme hakkı veren türev araçları,
    > 2. Değeri, bir menkul kıymet fiyatına veya getirisine; bir döviz fiyatına veya fiyat değişikliğine; faiz oranına veya orandaki değişikliğe; bir kıymetli maden veya kıymetli taş fiyatına veya fiyat değişikliğine; bir mal fiyatına veya fiyat değişikliğine; Kurulca uygun görülen kurumlarca yayınlanan istatistiklere veya bunlardaki değişikliğe; kredi riski transferi sağlayan, enerji fiyatları ve iklim değişkenleri gibi ölçüm değerleri olan ve bu sayılanlardan oluşturulan bir endeks seviyesine veya seviyedeki değişikliğe bağlı olan türev araçları, bu araçların türevlerini ve sayılan dayanak varlıkları birbirleri ile değiştirme hakkı veren türevleri,
    > 3. Döviz ve kıymetli madenler ile Kurulca belirlenecek diğer varlıklar üzerine yapılacak kaldıraçlı işlemleri,
    
    > fade etmektedir. `SPK M. 3, u`

    ***

3. **Sermaye piyasası araçları:** 
    >Menkul kıymetler ve türev araçlar ile yatırım
sözleşmeleri de dâhil olmak üzere Kurulca bu kapsamda olduğu belirlenen diğer sermaye
piyasası araçlarını 
    
    > ifade etmektedir. `SPK M. 3, ş`

***

4. **Değer artışı kazançları:**
    > Aşağıda yazılı olup geçen bölümlerin dışında kalan kazanç ve iratlar bu bölümdeki hükümlere göre vergiye tâbi gelire dahildir:

    > Değer artışı kazançları. `GVK M. 80`

5.  **TEFAS:**
    Türkiye Elektronik Fon Alım Satım Platformu `TEFAS` Türkiye’deki tüm yatırım fonlarının (bankalar ve portföy yönetim şirketleri tarafından kurulan) tek bir merkezden alınıp satılabildiği merkezi bir elektronik pazar yeridir. Ayri olarak tanimi bulunmasada `GVK Geçici M. 67, 1, d`'de su sekilde gecmektedir:

    > ... Hazine ve 4749 sayılı Kanuna göre kurulan varlık kiralama şirketleri tarafından yurt dışında ihraç edilen menkul kıymetlerin alım satımı, itfası sırasında elde edilen getirileri ile bunların dönemsel getirilerinin tahsilinde, tam mükellef kurumlara ait olup, Borsa İstanbul’da işlem gören ve bir yıldan fazla süreyle elde tutulan hisse senetleri ile sürekli olarak portföyünün en az % 51'i Borsa İstanbul’da işlem gören hisse senetlerinden oluşan yatırım fonlarının (Ek hüküm:4/12/2025-7566/3 md.) (Katılma payları sadece nitelikli yatırımcılara satılabilen, Türkiye Elektronik Fon Alım Satım Platformu’nda işlem görmeyen ve fon portföyüne alınacak varlık ve işlemlere ilişkin herhangi bir oransal sınırlamaya tabi olmayan yatırım fonları hariç) bir yıldan fazla süreyle elde tutulan katılma paylarının elden çıkarılmasında ve hisse senetleri kâr paylarının hisse sahipleri adına tahsilinde bu fıkra hükümleri uygulanmaz. Tam mükellef kurumlara ait olup, Borsa İstanbul’da işlem gören ve bir yıldan fazla süreyle elde tutulan hisse senetlerinin elden çıkarılmasından elde edilen gelirler için, Gelir Vergisi Kanununun Mükerrer 80 inci madde hükümleri uygulanmaz. ...

6. **KAP:** 
    Kamuyu Aydınlatma Platformu `KAP`, sermaye piyasasında işlem gören şirketlerin ve yatırım fonlarının, yatırımcıları bilgilendirmekle yükümlü oldukları tüm verileri paylaştıkları resmi ve elektronik bir veri deposudur.

***
    Menkul Kıymet ⊆ Sermaye Piyasası Aracı ⊆ Menkul Sermaye

## Araçlar

1. ### Hisse Senedi: 
    Menkul kiymet sayilir. Elden cikarilmasi sonucu olusan kar **Değer artışı kazanci** olarak degerlendirilir.

    > Aşağıda yazılı mal ve hakların elden çıkarılmasından doğan kazançlar değer artışı kazançlarıdır: 
    
    > İvazsız olarak iktisap edilenler ile tam mükellef kurumlara ait olan ve iki yıldan fazla süreyle elde tutulan hisse senetleri hariç, menkul kıymetlerin veya diğer sermaye piyasası araçlarının elden çıkarılmasından sağlanan kazançlar. `GVK Mükerrer M. 80, 1`

   BIST'de yaplian hisse senedi islemleri stopaja dahildir. Yani Beyan gerekmez. BIST disi kaynaklanan yurt disi piyasa kazanclari ise yillik beyana tabidir.

    Dolaysiyla beyan edilmesi gerektigi durumda GİB uzerinden `3.D` yerinde `GVK Madde 80.1`'si seciniz.

    ***

    #### Temettü
    Hisse senedinden elde edilen temettüler ise Menkul Sermaye İradı sayılır.
    
    > Her nevi hisse senetlerinin kâr payları (kurucu hisse senetleri ve diğer intifa hisse senetlerine verilen kâr payları ve pay sahiplerine hazırlık dönemi için faiz olarak veya başka adlarla yapılan her türlü ödemeler ile Sermaye Piyasası Kanununa göre kurulan yatırım fonları katılma belgelerine ödenen kâr payları ile konut finansmanı fonlarını temsilen ihraç edilen ipoteğe dayalı menkul kıymetlere ve varlık finansmanı fonlarını temsilen ihraç edilen varlığa dayalı menkul kıymetlere ödenen faiz, kâr payı veya benzeri gelirler dahil.) `GVK M. 75, 1`

    #### Yurt Ici

    > Tam mükellef kurumlardan elde edilen, 75 inci maddenin ikinci fıkrasının (1), (2) ve (3) numaralı bentlerinde yazılı kâr paylarının yarısı gelir vergisinden müstesnadır. İstisna edilen tutar üzerinden 94 üncü madde uyarınca tevkifat yapılır ve tevkif edilen verginin tamamı, kâr payının yıllık beyanname ile beyan edilmesi durumunda yıllık beyanname üzerinden hesaplanan vergiden mahsup edilir. `GVK M. 22, 3`
 
    Yani vergilendirilecek gelir aslinda gercek gelirin yarisi olacak.

    > Aşağıda belirtilen gelirler için yıllık beyanname verilmez, diğer gelirler için beyanname verilmesi halinde bu gelirler beyannameye dahil edilmez: 

    > Vergiye tâbi gelir toplamının [(a) ve (b) bentlerinde belirtilenler hariç] 103 üncü maddede yazılı tarifenin ikinci gelir diliminde yer alan tutarı aşmaması koşuluyla, Türkiye'de tevkifata tâbi tutulmuş olan; birden fazla işverenden elde edilen ücretler, menkul sermaye iratları ve gayrimenkul sermaye iratları. `GVK M. 86, c`

    Dolaysiyla karinizin yarisi bu limiti gecmiyor ise vergi verilmez. 

    Beyan edilmesi gerektigi durumda GİB uzerinden `3.C` yerinde `606` seciniz.

    #### Yurt Disi

    > Aşağıda belirtilen gelirler için yıllık beyanname verilmez, diğer gelirler için beyanname verilmesi halinde bu gelirler beyannameye dahil edilmez: 

    > Bir takvim yılı içinde elde edilen ve toplamı 600 milyon lirayı (22.000 TL) aşmayan, tevkifata ve istisna uygulamasına konu olmayan menkul ve gayrimenkul sermaye iratları. `GVK M. 86, d`
    
    Not: Bu limit yillik olarak guncellenmektedir. Lutfen guncel limite bakiniz.

    Beyan edilmesi gerektigi durumda GİB uzerinden `3.C` yerinde `622` seciniz.

    
***
***
    

2. ### Kripto:
    > Kripto varlık: Dağıtık defter teknolojisi veya benzer bir teknoloji kullanılarak elektronik olarak oluşturulup saklanabilen, dijital ağlar üzerinden dağıtımı yapılan ve değer veya hak ifade edebilen gayri maddi varlıkları,

    > ifade eder. `SPK M. 3, bb`

    Tanimi yapilmis olsa da deger artisi kazancinda yer almamaktadir. Yani vergisi simdilik yoktur, ancak tasarim asamasinda oldugu icin yakin zamanda yururluge girebilir.

***
***

3. ### Yatırım Fonu:

    Sermaye piyasası aracıdır. Bir fon eger KAP veya TEFAS Yatirim Fonu bolumunde bulunuyorsa Turkiye de bu fon tanimlanmis demektir. Yani islem yapildiginda vergi stopaj olarak kesilmek zorundadir bu fon için. Eger:
    1. Kod 3 harfli,
    2. TL ile alınıyor,
    3. `Yatırım Fonları` sekmesinde,

    ise bu fon yuksek ihtimalle KAP veya TEFAS Yatirim Fonu bolumunde bulunmaktadir. Eger emin degilseniz [KAP](https://kap.org.tr/tr/YatirimFonlari/YF) veya [TEFAS](https://www.tefas.gov.tr/FonKarsilastirma.aspx) uzerinden sorgulayabilirsiniz. Fon temettu veriyorsa stopaja dahildir. Fonlar hakkinda daha fazla bilgilendirme için `GVK Geçici M. 67`.

    ***

    Ancak fon KAP veya TEFAS yatirim fonu kimsinda bulunmuyor Türkiye’de kurulu olmayan bir yatırım fonu veya ETF ise yatırımcı tarafından beyan edilmelidir. Bu durumda bu fonun vergilendirilmesi yabanci hisse senetlerinin vergilendirilmesi ile birebir aynidir. `GVK Mükerrer M. 80, 1` ve `GVK M. 86, d`. [Hisse Senedi](#hisse-senedi) kismindan fonu yabanci hisseymis gibi dusunup vergi verebilirsiniz.

***
***


## Vergilendirme

Iki tur vergilendirme yolu vardir. **Stopaj** ve **Beyanname**. Stopaj, islem yapildigi anda gerceklesmektedir. Dolaysiyla ayri olarak beyan verilmesi gerkmez. Ancak Beyanname olmasi durumunda vergi vermesi gereken kisi olsuan kazancini beyan etmesi gerekir. 

Gelir vergisine dahil olan gelirlerin vergilendirilme oraninina `GVK M. 103`'ten ulasabilirsiniz.


## Kâr Hesaplama

Kar hesaplamizin genelde iki sebebi vardir: 

1. islem periyodu sonunda elde ettigimiz kar ve perfonmansi ogrenmek.
2. Vergi beyaninda bulunabilmek.

Eger kar bilinimiyorsa bu iki madde gerceklestirilemez. Sermaye piyasasi araclarinin alis-satis islemlerinden elde edilen gelirleri hesaplarken ilk giren ilk cikan `FIFO` yontemi kullanilir. 

> Aynı menkul kıymet ve diğer sermaye piyasası aracından değişik tarihlerde alımlar yapıldıktan sonra bunların bir kısmının elden çıkarılması halinde, **ilk giren ilk çıkar yöntemi** kullanılmak suretiyle, tevkifat matrahının tespitinde dikkate alınacak alış bedeli belirlenir. Bir menkul kıymet ve diğer sermaye piyasası aracının alımından önce elden çıkarılması halinde, elden çıkarılma tarihinden sonra yapılan ilk alım işlemi esas alınarak üzerinden tevkifat yapılacak tutar tespit edilir. Aynı gün içerisindeki işlemlerde ağırlıklı ortalama yöntemi kullanılabilir. Alış ve satış işlemleri dolayısıyla ödenen komisyonlar ile Banka ve Sigorta Muameleleri Vergisi tevkifat matrahının tespitinde dikkate alınır. `GVK Geçici M. 67, d`

### Nasil Yapilir?

Asagidaki maddeleri oncelikle bilmeniz gerekmektedir:

1. [Dövizle Endeksleme](#dövizle-endeksleme)
2. [Yİ-ÜFE Endeksleme](#yi̇-üfe-endeksleme)

#### Dövizle Endeksleme

> Kur farkından doğan kazançların hesabında, menkul kıymet veya iştirak hisselerinin iktisabına tahsis edilen yabancı sermayenin bu kıymet veya hisselerin iktisap tarihindeki Türkiye Cumhuriyet Merkez Bankası döviz alış kuruna göre hesaplanan Türk Lirası karşılığı ile bu kıymet veya hisselerin elden çıkarılması tarihindeki aynı miktar yabancı sermayenin Türkiye Cumhuriyet Merkez Bankası döviz alış kuruna göre hesaplanan Türk Lirası karşılığı arasındaki fark esas alınır. `GVK Mükerrer M. 81`

> Yabancı paralar borsa rayici ile değerlenir. Borsa rayicinin takarrüründe muvazaa olduğu anlaşılırsa bu rayiç yerine alış bedeli esas alınır. Yabancı paranın borsada rayici yoksa, değerlemeye uygulanacak kur Maliye Bakanlığınca tesbit olunur. `VUK M. 280`

Yani eger elimizdeki sermaye yabanci serymaye ise (islemi TL disinda bir doviz ile gerceklesiyor ise) kur farki uygulanmak zorundadir. Eger islemin gerceklestigi tarih ve saatteki kur-tl verisi sermayenin islem gordugu platformda bulunuyorsa bu veri kullanilir. Eger yoksa `TCMB` tarafindan aciklanan, islemin gerceklesitgi tarihin bir onceki is gunu kur-tl verisi kullanilir. Yukarida Maliye Bakanlığınca denilmesinin sebebi Maliye Bakanlıği degeri belirler, veriyi `TCMB` aciklar. Kur alış baz alınır.

$$
M_{TL} =  M_{X} \times  R
$$
$$
M_{TL}: \text{TL Maliyet} \\[1ex]
M_{X}: \text{X Kuru Maliyet}\\[1ex]
R_{X-TL}: \text{Döviz Alış kuru}\\
$$

##### Örnek:

|               | 9 Şubat | 10 Şubat | 19 Şubat | 20 Şubat |
| -----------   | ----------- | ----------- | ----------- | ----------- |
| Dolar-Tl      | 40,00| 41,37 | 45,00  | 43,92 |
| Nvdia Hissesi | 98$  | 100$  | 188.1$ | 200$  |

    Not: Hisse değerleri işlem anındaki değeri ile kullanılmalıdır. Örneği basitleştirmek amacıyla burada günlük tarih kullanılmıştır.

Diyelimki 10 Şubat'ta 5 adet Nvdia hissesi aldık. Yani elimizde $\text{100\$} \times 5 = \text{500\$}$ var. Bu hisselerin hepsini de 20 Şubat'ta satmış olalım $\text{200\$} \times 5 = \text{1.500\$}$. Varsayalım ki işlem yaptığımız platform bize işlem anındaki Dolar-Tl değerini sağlamıyor. Bu yüzden bir önceki iş günlerinin değerlerini kullanıyor olacağız (9 Şubat Pazartesi günü olsun). 9 ve 19 Şubat iş günleri oldukları için bu verileri kullanacağız.

Hisse Alış TL:

$$
\text{500\$} \times 40,00 = \text{20.000₺}
$$

Hisse Satış Tl:

$$
\text{1.000\$} \times 45,00 = \text{45.000₺}
$$

Kar:
$$
\text{45.000₺} - \text{20.000₺} = \text{25.000₺}
$$

***

#### Yİ-ÜFE Endeksleme

> Mal ve hakların elden çıkarılmasında iktisap bedeli, elden çıkarılan mal ve hakların, elden çıkarıldığı ay hariç olmak üzere Devlet İstatistik Enstitüsünce belirlenen toptan eşya fiyat endeksindeki artış oranında artırılarak tespit edilir. (Ek cümle: 30/12/2004-5281/28 md.) Şu kadar ki, bu endekslemenin yapılabilmesi için artış oranının % 10 veya üzerinde olması şarttır. `GVK Mükerrer M. 81`

Dolayısıyla işlemlerin gerçekleştiği tarihten 1 ay önceki tarihlerdeki endeks değerleri kullanılır. Endeks değeri 10% oranı ve üzerinde artmışsa bu artış yüzdesini endeksleyerek uygulamamıza izin verilir.


$$
k = \frac{E_{sell}}{E_{buy}} - 1
$$
$$
M_E = M\times (k + 1)
$$
$$
E_{sell}: \text{Endeks Satış Değeri}\\
E_{buy}: \text{Endeks Alış Değeri}\\
k: \text{Endeks Katsayısı}\\
M_E: \text{Endeksli Maliyet}\\
M: \text{Maliyet}\\
$$
$$
\text{Eğer } k \geq 0.10 \text{ ise endeksleme yapılır. } \\ \text{Eğer } k < 0.10 \text{ ise endeksleme yapılmaz.}
$$


##### Örnek:

|               | 2020 Şubat | 2020 Mart | 2023 Kasım | 2023 Aralık |
| -----------   | ----------- | ----------- | ----------- | ----------- |
| Tİ-UFE        | 400,00 | 432,29 | 2000,00  | 2037,45 |
| Tuprs Hissesi  | 19,38₺    | 50₺   | 183,1₺ | 210₺  |

    Not: Tİ-ÜFE değerleri aylık olarak açıklanır, o yüzden bu örnekte tarih değerleri yıl ve ay olarak kullanılmıştır.

Diyelimki 2020 Mart ayında 10 adet Tüpraş hissesi aldık. Yani elimizde $50₺ \times 10 = 500₺$ var. Bu hisselerin hepsini de 2023 Aralık'ta satmış olalım $210₺ \times 10 = 2.100₺$. 2020 Şubat ve 2023 Kasım işlem tarihlerinden bir önceki aylar oldukları için bu verileri kullanacağız.

Öncekilikle endeksin artış oranını kontrol ediyoruz.

$$
\frac{2000,00}{400,00} - 1 = 4
$$


Endeks değerimiz 400% oranında artmış (değeri 5 katı olmuş) bulunmakta (son değeri 100 ile çarparak yüzdelik değeri hesaplayabilirsiniz).  
4 > 0.1 olduğu için endeksleme yapabiliriz.

Hisse Alış Endeksli:

$$
\text{500₺} \times (4 + 1) = \text{2.500₺}
$$

Bu değeri şöyle düşünün, 2020 Mart tarihindeki paramızın 2023 Aralık tarihindeki para değeri 2.500₺'dir.

Kar:

$$
\text{2.100₺} - \text{2.500₺} = -\text{400₺}
$$


Paramız sayısal olarak artmış gözükse de aslında enflasyona karşı değer kaybetmiştir.